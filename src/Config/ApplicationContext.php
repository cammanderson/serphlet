 ...
 */
class ApplicationContext implements ServletContextInterface
{
     ...
     */
    public function addFilterDef(\Serphlet\Config\FilterDef $filterDef)
    {
     ...
     */
    public function removeFilterDef(\Serphlet\Config\FilterDef $filterDef)
    {
     ...
     *  is malformed
     */
    public function addFilterMaps(\Serphlet\Config\FilterMaps $filterMaps)
    {
        ...
        foreach ($servletNames as $servletName) {
            $fmap = new \Serphlet\Config\FilterMap();
    ...
    public function filterStart()
    {
        ...
        foreach (array_keys($filterDefs) as $name) {
            try {
                $filterConfig = new \Serphlet\Config\FilterConfig($this, $filterDefs[$name]);
...
//            log->warn(sm->getString("standardContext->crlfinurl", urlPattern));
    ...
    public function getRequestDispatcher($path)
    {
        if (is_null($this->requestDispatcher)) {
            $this->requestDispatcher = new \Serphlet\Application\RequestDispatcher($this);
        }
    ...
    public function setRequestDispatcher(\Serphlet\Request\Dispatcher $dispatcher)
    {
    ...
    public function addServletConfig(\Serphlet\Config\ApplicationConfig $servletConfig)
    {
        $servletConfig->setServletContext($this);
        $this->servlets[$servletConfig->getServletName()] = $servletConfig;
    }
    public function addServletMapping(\Serphlet\Config\ServletMap $servletMapping)
    {
        // Locate the servlet
    ...
    public function addErrorPage($errorPage)
    {
        ...
        if (!trim($errorPage->getLocation()) || substr($errorPage->getLocation(), 0, 1) != '/') {
            throw new \Serphlet\Exception\IllegalArgument("servletConfig->errorPage->location url location not valid");
        }
